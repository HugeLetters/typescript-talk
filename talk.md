...
Бивариативность, это когда имея дженерик `F<T>` и при условии что `A` является подтипом `B`, то `F<A>` является подтипом `F<B>`, и `F<B>` является подтипом `F<A>`, одновременно. Если два типа являются подтипами друг друга одновременно, то как правило это означает что это один и тот же тип - аналогично тому если `a <= b` и `b <= a`, то очевидно, что `a = b`. В случае дженерика `Null<T>` это действительно так, ведь он всегда возвращает один и тот же тип. В непротиворечивой системе типизации такой вид дженериков это единственный случай, в котором бы наблюдалась бивариативность.
