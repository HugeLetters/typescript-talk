...
Но к неизменяемым массивам это не относится - они взаправду ковариативны. Можно задаться вопросом насчет методов типа `concat` - ведь они есть и на неизменяемых массивах, принимают на вход только тип элемента массива. По идее мы должны тогда наблюдать ту же ситуацию с контрвариативностью на методах.

Дело в том, что так как `concat` возвращает новый массив, то на вход он может принимать любые значения - следовательно, никакой контрвариативности на подобных методах наблюдаться не будет. Тайпскрипт просто решил не переусложнять аннотацию этого метода, но на самом деле, ее можно было бы расширить, чтобы передавать можно было любые значения.

# Conclusion

Это две концепции - то, что типы это множества, и вариативность с ее подвидами, - это основы, которые я думаю, дают хорошую основу для понимания более продвинутых фичей в Тайпскрипте. Это далеко не все, что есть - я не затрагивал дистрибьютивность, `infer`, `satisfies`, `template literals`, `conditional types` или `higher-kinded types`. Но это уже все же конкретные фичи - я хотел рассказать про две главных абстрактных идеи, которые сильно помогут разобраться со всем остальным.
